...
const results = useWordsSearch(debouncedQuery)
...
const { fetchRandom, randomData } = useRandomWord()

async function goToRandom() {
  await fetchRandom()
  if (randomData.value?.term) {
    navigateTo(`/w/${encodeURIComponent(randomData.value.term)}`)
  }
}
