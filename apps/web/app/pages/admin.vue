...
const { submitEntry, error: submitError } = useSubmitEntry()
...
async function handleSubmit() {
  ...
  if (submitError.value) {
    toast.add({ title: (submitError.value as { data?: { message?: string } })?.data?.message || 'Failed to save entry', color: 'error' })
    return
  }
