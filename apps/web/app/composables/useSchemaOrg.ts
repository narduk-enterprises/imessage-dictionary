...
export function useArticleSchema(options: ArticleOptions) {
  ...
  const articleConfig: Record<string, unknown> = {
    headline,
    description,
    datePublished,
  ...
  if (image) articleConfig.image = image
  if (section) articleConfig.articleSection = section
  if (tags) articleConfig.keywords = tags

  useSchemaOrg([defineArticle(articleConfig)])
}
